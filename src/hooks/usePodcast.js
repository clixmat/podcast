...
export function usePodcast({ repository, filter, podcastId }) {
    const [repositoryData, setRepositoryData] = useState([])
    const { setLoaderPodcastActive } = useLoader()
    const [topPodcastsLocalStorage, setTopPodcastLocalStorage] = useLocalStorage('podcasts', undefined)
    ...
    const filterPodcasts = useCallback((filter) => {
        ...
        if (filter === '') setRepositoryData(topPodcastsLocalStorage)
        else {
