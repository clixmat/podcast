...
export function usePodcastEpisode({ repository, podcastId, episodeId }) {
    ...
    useEffect(() => {
        ...
        if (podcastsEpisodeLocalStorage) {
            const episodes =
                podcastId && episodeId
                    ? repository.getEpisodeById(podcastsEpisodeLocalStorage, podcastId, episodeId)
                    : podcastsEpisodeLocalStorage
